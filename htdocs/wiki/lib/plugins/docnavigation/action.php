...
class action_plugin_docnavigation extends DokuWiki_Action_Plugin {
    ...
    public function _addtopnavigation(Doku_Event &$event, $param) {
        global $ACT;
        //if($ACT != 'show') return;
        if($event->data[0] != 'xhtml' || !in_array($ACT, array('show'))) return;

        $event->data[1] = $this->getNavbar($showtoc = false).$event->data[1];
    }
    ...
    public function _addbottomnavigation(Doku_Event &$event, $param) {
        ...
        if($pos) {
            $event->data = substr_replace($event->data, $this->getNavbar($showtoc = true), $pos, 0);
        } else {
            $event->data = $event->data.$this->getNavbar($showtoc = true);
        }
    }
     ...
     *
     * @param bool $showtoc add referer to ToC
     * @return string
     */
    private function getNavbar($showtoc = true) {
        global $ID;
        ...
        if(!empty($data)) {
            ...
            if($showtoc) $out .= '<div class="clearer"></div>'.DOKU_LF;

            $out .= '<div class="docnavbar'.($showtoc ? ' showtoc' : '').'">'.DOKU_LF.DOKU_TAB.'<div class="leftnav">';
            if($data['previous'][0]) $out .= '← '.$renderer->internallink($data['previous'][0], $data['previous'][1], null, true);
            $out .= '&nbsp;</div>'.DOKU_LF;

            if($showtoc) {
                $out .= DOKU_TAB.'<div class="centernav">';
                if($data['toc'][0]) $out .= $renderer->internallink($data['toc'][0], $data['toc'][1], null, true);
                $out .= '&nbsp;</div>'.DOKU_LF;
            }

            $out .= DOKU_TAB.'<div class="rightnav">&nbsp;';
            if($data['next'][0]) $out .= $renderer->internallink($data['next'][0], $data['next'][1], null, true).' →';
