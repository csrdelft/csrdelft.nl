...
jQuery(document).ready(function ($) {
	...
	$('.item-has-children').children('a').on('click', function (event) {
		...
		$(this).toggleClass('toggle-group-open').next('.sub-menu').slideToggle(200).end().parent('.item-has-children').siblings('.item-has-children').children('a').removeClass('toggle-group-open').next('.sub-menu').slideUp(200);
	});
