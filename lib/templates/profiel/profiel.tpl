<div id="profiel" {if $profiel->isJarig()}class="jarig"{/if}>
					...
					<div class="data">
						<table id="recenteForumberichten">
							...
							{foreach from=$posts_draden[0] item=post}
								<tr>
									<td><a href="/forumpost/{$post->post_id}" title="{$post->tekst}"{if !$posts_draden[1][$post->draad_id]->alGelezen()} class="opvallend"{/if}>{$posts_draden[1][$post->draad_id]->titel|truncate:75|escape:'html'}</a></td>
									<td>
	...
	{if ($isAdmin OR $isLidMod) AND $profhtml.changelog!=''}
		<div class="profielregel" id="changelog">
			<div class="gegevens">
				<div class="handje" onclick="$('#changelog_gegevens').toggle();
