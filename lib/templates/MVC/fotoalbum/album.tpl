	...
	<div id="{$foto->filename|md5}" class="thumb hoverIntent">
		{if LoginModel::mag('P_DOCS_MOD')}
			<div style="position: absolute;">
				...
				<a href="/fotoalbum/roteren/{$album->getSubDir()}" postdata="foto={$foto->filename}&rotate=-90" class="knop post hoverIntentContent" title="Foto tegen de klok in draaien" style="position: relative; top: 130px; left: -40px;">{icon get=arrow_rotate_anticlockwise}</a>
				<a href="/fotoalbum/roteren/{$album->getSubDir()}" postdata="foto={$foto->filename}&rotate=90" class="knop post hoverIntentContent" title="Foto met de klok mee draaien" style="position: relative; top: 130px; left: 50px;">{icon get=arrow_rotate_clockwise}</a>
			</div>
		{/if}
