...
{if LoginModel::mag('P_ADMIN')}
	<a href="/menubeheer/bewerken/{$root->item_id}" data="menu={$root->tekst}" class="knop post modal" title="Dit menu bewerken">{icon get="bewerken"}</a>
{/if}
...
{if !$root->children AND LoginModel::mag('P_ADMIN')}
	<div class="float-right">
		<a href="/menubeheer/verwijderen/{$root->item_id}" data="menu={$root->tekst}" class="knop post confirm ReloadPage" title="Dit menu definitief verwijderen">{icon get="cross"}</a>
	</div>
{/if}
<hr />