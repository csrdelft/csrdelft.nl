...
<li id="menu-item-{$item->item_id}" parentid="{$item->parent_id}" class="menu-item">
	{$item->prioriteit}
	<a href="/menubeheer/bewerken/{$item->item_id}" class="knop post popup" title="Dit agenda-item bewerken">{icon get="bewerken"}</a>
	<a href="/menubeheer/toevoegen/{$item->item_id}" class="knop post popup" title="Sub-menu-item toevoegen">{icon get="add"}</a>
