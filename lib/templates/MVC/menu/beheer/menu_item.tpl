...
<li id="menu-item-{$item->item_id}" parentid="{$item->parent_id}" class="menu-item">
	...
	<a href="/menubeheer/bewerken/{$item->item_id}" class="knop post modal" title="Dit menu-item bewerken">{icon get="bewerken"}</a>
	...
	<div class="float-right">
		...
		<a href="/menubeheer/verwijderen/{$item->item_id}" class="knop post confirm ReloadPage" title="Dit menu-item definitief verwijderen">{icon get="cross"}</a>
	</div>
	{if $item->children}
