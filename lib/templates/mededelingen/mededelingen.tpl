{if $model->isModerator()}
	...
	<hr/>
{/if}
<div id="mededelingenketser" class="mededelingen container">
	{if $prullenbak}
	...
	<div class="row">
		...
		<div id="kolomlinks" class="col-md-4">
			{* Knoppen bovenaan *}
			...
			{if $model->isModerator()}
				...
				<div id="legenda" class="dragobject verborgen">
					...
					<ul>
						...
						{if $prullenbak}
							<li><span class="lichtgrijs">grijs gekleurde</span> mededelingen zijn verborgen en dus alleen zichtbaar in
								de prullenbak
							</li>
							<li><span class="error">rood gekleurde</span> mededelingen zijn verwijderd en dus alleen zichtbaar in de
								prullenbak
							</li>
						{/if}
					</ul>
					...
					{foreach from=$wachtGoedkeuring key=groepering item=mededelingen}
						<div class="mededelingenlijst-block">
							...
							{foreach from=$mededelingen item=mededeling}
								<div {if $mededeling->id==$geselecteerdeMededeling->id}id="actief"
										 {/if}class="mededelingenlijst-item{if $mededeling->verborgen} verborgen-item{/if}">
									...
									<div class="itemtitel">
										{* {$mededeling->getDatum()} *}
										<a
											href="{$pagina_root}{$mededeling->id}"{if $model->isModerator()} class="{if !$mededeling->prive}cursief{/if} {if $mededeling->zichtbaarheid=='wacht_goedkeuring'}dikgedrukt{/if}"{/if}>{$mededeling->titel|bbcode|html_substr:"40":"…"}</a>
									</div>
								</div>
							{/foreach}
						</div>
						{* Einde mededelingenlijst-block*}
					{/foreach}
				</div>
			{/if}
		</div> {* Einde kolom links *}

		<div id="kolomrechts" class="col-md-8">
			{* De mededeling rechtsbovenaan *}
			<div class="nieuwsbericht">
				<div class="nieuwsbody">
					<div class="nieuwstitel">{$geselecteerdeMededeling->titel|escape:'html'}</div>
					<img class="nieuwsplaatje" src="/plaetjes/mededelingen/{$geselecteerdeMededeling->plaatje}" width="200px"
							 height="200px" alt="{$geselecteerdeMededeling->plaatje}"/>
					{$geselecteerdeMededeling->tekst|bbcode}<br/>
				</div>
				<div class="informatie">
					<hr/>
					Geplaatst
					op {$geselecteerdeMededeling->datum|date_format:'%d-%m-%Y'}{if $model->isModerator()} door {$geselecteerdeMededeling->getProfiel()->getLink('civitas')}{/if}
					<br/>
					Categorie: {$geselecteerdeMededeling->getCategorie()->naam}<br/>
					{if $model->isModerator()}
						Doelgroep: {$geselecteerdeMededeling->doelgroep}
						<br/>
						Prioriteit: {$geselecteerdeMededeling->prioriteit}
						<br/>
					{/if}
					{if $geselecteerdeMededeling->magBewerken()}
						Vervalt {if $geselecteerdeMededeling->vervaltijd===null}nooit{else}op: {$geselecteerdeMededeling->vervaltijd|date_format:$datumtijdFormaat}{/if}
						<br/>
					{/if}
					...
					{if $geselecteerdeMededeling->magBewerken()}
						...
						<a href="{$pagina_root}verwijderen/{$geselecteerdeMededeling->id}"
							 onclick="return confirm('Weet u zeker dat u deze mededeling wilt verwijderen?');">
							{icon get="verwijderen"}
						</a>
						{if $model->isModerator() AND $geselecteerdeMededeling->zichtbaarheid=='wacht_goedkeuring'}
							<a onclick="return confirm('Weet u zeker dat u deze mededeling wilt goedkeuren?')"
								 href="{$pagina_root}goedkeuren/{$geselecteerdeMededeling->id}">
								{icon get="goedkeuren"}
							</a>
						{/if}
					{/if}
