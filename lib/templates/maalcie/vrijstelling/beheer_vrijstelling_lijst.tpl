...
<tr id="vrijstelling-row-{$vrijstelling->getUid()}">
	<td>
		<a href="{$smarty.const.maalcieUrl}/bewerk/{$vrijstelling->getUid()}" title="Vrijstelling wijzigen" class="knop rounded post modal">{icon get="pencil"}</a>
	</td>
	...
	<td class="col-del">
		<a href="{$smarty.const.maalcieUrl}/verwijder/{$vrijstelling->getUid()}" title="Vrijstelling definitief verwijderen" class="knop rounded post confirm">{icon get="cross"}</a>
	</td>
</tr>