...
<tr id="vrijstelling-row-{$vrijstelling->getUid()}">
	<td>
		<a href="{Instellingen::get('taken', 'url')}/bewerk/{$vrijstelling->getUid()}" title="Vrijstelling wijzigen" class="knop post modal">{icon get="pencil"}</a>
	</td>
	...
	<td class="col-del">
		<a href="{Instellingen::get('taken', 'url')}/verwijder/{$vrijstelling->getUid()}" title="Vrijstelling definitief verwijderen" class="knop post confirm">{icon get="cross"}</a>
	</td>
</tr>