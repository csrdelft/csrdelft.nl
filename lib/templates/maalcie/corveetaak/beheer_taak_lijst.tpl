...
{if $taak->getIsVerwijderd()}
		<a href="{maalcieUrl}/herstel/{$taak->getTaakId()}" title="Corveetaak herstellen" class="knop rounded post">{icon get="arrow_undo"}</a>
{else}
		<a href="{maalcieUrl}/bewerk/{$taak->getTaakId()}" title="Taak wijzigen" class="knop rounded post modal">{icon get="pencil"}</a>
...
{if !$taak->getIsVerwijderd() and (!isset($maaltijd) or !$maaltijd->getIsVerwijderd())}
	...
	{if $taak->getUid()}
		<a href="{maalcieUrl}/email/{$taak->getTaakId()}" title="Verstuur een (extra) herinnering voor deze taak" class="knop rounded post confirm">
	{/if}
{/if}
...
{if isset($wijzigbaar) and $taak->getUid()}
	...
	{if $taak->getWanneerToegekend()}
		<a href="{maalcieUrl}/puntenintrekken/{$taak->getTaakId()}" title="Punten intrekken" class="knop rounded post">{icon get="medal_silver_delete"}</a>
	{else}
		<a href="{maalcieUrl}/puntentoekennen/{$taak->getTaakId()}" title="Punten toekennen" class="knop rounded post">{icon get="award_star_add"}</a>
	{/if}
{/if}
		</div>
	</td>
	<td class="col-del">
{if $taak->getIsVerwijderd()}
		<a href="{maalcieUrl}/verwijder/{$taak->getTaakId()}" title="Corveetaak definitief verwijderen" class="knop post confirm range"><input type=checkbox id="box-{$taak->getTaakId()}" name="del-taak" />{icon get="cross"}</a>
{else}
		<a href="{maalcieUrl}/verwijder/{$taak->getTaakId()}" title="Corveetaak naar de prullenbak verplaatsen" class="knop post range"><input type=checkbox id="box-{$taak->getTaakId()}" name="del-taak" />{icon get="bin_closed"}</a>
{/if}
	</td>
</tr>
{/strip}
