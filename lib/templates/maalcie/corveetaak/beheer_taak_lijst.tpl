...
{if !isset($show) and !$prullenbak} verborgen{/if}">
	<td>
{if $taak->verwijderd}
		<a href="/corvee/beheer/herstel/{$taak->taak_id}" title="Corveetaak herstellen" class="btn post">{icon get="arrow_undo"}</a>
{else}
		<a href="/corvee/beheer/bewerk/{$taak->taak_id}" title="Taak wijzigen" class="btn post popup">{icon get="pencil"}</a>
...
{if !isset($maaltijd) and $taak->maaltijd_id}
	<a href="/corvee/beheer/maaltijd/{$taak->maaltijd_id}" title="Beheer maaltijdcorvee" class="btn">{icon get="cup_link"}</a>
{/if}
	</td>
...
{if !$taak->verwijderd and (!isset($maaltijd) or !$maaltijd->verwijderd)}
	...
	{if $taak->uid}
		<a href="/corvee/beheer/email/{$taak->taak_id}" title="Verstuur een (extra) herinnering voor deze taak" class="btn post confirm">
	{/if}
{/if}
...
{if isset($wijzigbaar) and $taak->uid}
	...
	{if $taak->wanneer_toegekend}
		<a href="/corvee/beheer/puntenintrekken/{$taak->taak_id}" title="Punten intrekken" class="btn post">{icon get="medal_silver_delete"}</a>
	{else}
		<a href="/corvee/beheer/puntentoekennen/{$taak->taak_id}" title="Punten toekennen" class="btn post">{icon get="award_star_add"}</a>
	{/if}
{/if}
		</div>
	</td>
	<td class="col-del">
{if $taak->verwijderd}
		<a href="/corvee/beheer/verwijder/{$taak->taak_id}" title="Corveetaak definitief verwijderen" class="btn post confirm range"><input type=checkbox id="box-{$taak->taak_id}" name="del-taak" />{icon get="cross"}</a>
{else}
		<a href="/corvee/beheer/verwijder/{$taak->taak_id}" title="Corveetaak naar de prullenbak verplaatsen" class="btn post range"><input type=checkbox id="box-{$taak->taak_id}" name="del-taak" />{icon get="bin_closed"}</a>
{/if}
	</td>
</tr>
{/strip}
