...
{/if}
	...
	<td class="niet-dik taak-{if $taak->uid}toegewezen{elseif  $taak->getBeginMoment() < strtotime(Instellingen::get('corvee', 'waarschuwing_taaktoewijzing_vooraf'))}warning{else}open{/if}">
{if isset($wijzigbaar)}
...
{/if}
{if $taak->uid}
		&nbsp;{ProfielModel::getLink($taak->uid, Instellingen::get('corvee', 'weergave_ledennamen_beheer'))}
{/if}
	</td>
	<td{if $taak->uid and ($taak->punten !== $taak->punten_toegekend or $taak->bonus_malus !== $taak->bonus_toegekend) and $taak->getBeginMoment() < strtotime(Instellingen::get('corvee', 'waarschuwing_puntentoewijzing_achteraf'))} class="taak-warning"{/if}>
