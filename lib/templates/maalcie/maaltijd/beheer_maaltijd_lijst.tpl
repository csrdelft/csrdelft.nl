...
{elseif $maaltijd->getIsVerwijderd()}
		<a href="{$smarty.const.maalcieUrl}/herstel/{$maaltijd->getMaaltijdId()}" title="Maaltijd herstellen" class="knop rounded post">{icon get="arrow_undo"}</a>
{else}
		<a href="{$smarty.const.maalcieUrl}/bewerk/{$maaltijd->getMaaltijdId()}" title="Maaltijd wijzigen" class="knop rounded post modal">{icon get="pencil"}</a>
...
{if !$maaltijd->getIsVerwijderd() and $maaltijd->getArchief() === null}
		<div class="float-right">
			<a href="{$smarty.const.maalcieUrl}/anderaanmelden/{$maaltijd->getMaaltijdId()}" title="Aanmelding toevoegen" class="knop rounded post modal">{icon get="user_add"}</a>
			<a href="{$smarty.const.maalcieUrl}/anderafmelden/{$maaltijd->getMaaltijdId()}" title="Aanmelding verwijderen" class="knop rounded post modal">{icon get="user_delete"}</a>
		</div>
{/if}
...
{else}
	{if $maaltijd->getIsGesloten()}
	<td class="maaltijd-gesloten">
		<a href="{$smarty.const.maalcieUrl}/open/{$maaltijd->getMaaltijdId()}" title="Heropen deze maaltijd" class="knop post">Gesloten</a>
	{else}
	<td class="maaltijd-open">
		<a href="{$smarty.const.maalcieUrl}/sluit/{$maaltijd->getMaaltijdId()}" title="Sluit deze maaltijd" class="knop post">Open</a>
	{/if}
{/if}
	</td>
	<td class="col-del">
{if $maaltijd->getIsVerwijderd()}
		<a href="{$smarty.const.maalcieUrl}/verwijder/{$maaltijd->getMaaltijdId()}" title="Maaltijd definitief verwijderen" class="knop post confirm range"><input type=checkbox id="box-{$maaltijd->getMaaltijdId()}" name="del-maaltijd" />{icon get="cross"}</a>
{else}
		<a href="{$smarty.const.maalcieUrl}/verwijder/{$maaltijd->getMaaltijdId()}" title="Maaltijd naar de prullenbak verplaatsen" class="knop post range"><input type=checkbox id="box-{$maaltijd->getMaaltijdId()}" name="del-maaltijd" />{icon get="bin_closed"}</a>
{/if}
	</td>
</tr>
{/strip}