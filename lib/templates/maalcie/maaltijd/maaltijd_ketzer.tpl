		...
		{if LoginModel::mag('P_LOGGED_IN')}
			<div class="aanmelddata maaltijd-{if $aanmelding}aan{else}af{/if}gemeld">Aangemeld:<br />
				...
				{if !$maaltijd->getIsGesloten() && LoginModel::mag('P_MAAL_IK')}
					...
					{if $aanmelding}
						<a onclick="ketzer_ajax('/maaltijdenketzer/afmelden/{$maaltijd->getMaaltijdId()}', '#maaltijdketzer-{$maaltijd->getMaaltijdId()}');" class="knop rounded maaltijd-aangemeld"><input type="checkbox" checked="checked" /> Ja</a>
					...
					{else}
						<a onclick="ketzer_ajax('/maaltijdenketzer/aanmelden/{$maaltijd->getMaaltijdId()}', '#maaltijdketzer-{$maaltijd->getMaaltijdId()}');" class="knop rounded maaltijd-afgemeld"><input type="checkbox" /> Nee</a>
