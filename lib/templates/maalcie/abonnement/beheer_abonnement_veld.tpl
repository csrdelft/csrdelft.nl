...
<td id="abonnement-cell-{$vanuid}-{$abonnement->mlt_repetitie_id}"
	...
	<a href="/maaltijden/abonnementen/beheer/{if $uid}uit{else}in{/if}schakelen/{$abonnement->mlt_repetitie_id}/{$vanuid}" class="btn post abonnement-{if $uid}in{else}uit{/if}geschakeld">
		<input type="checkbox"
