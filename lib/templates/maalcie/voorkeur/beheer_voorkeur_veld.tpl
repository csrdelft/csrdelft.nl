...
<td id="voorkeur-cell-{$voorkeur->getVanUid()}-{$crid}"
	...
	<a href="{$smarty.const.maalcieUrl}/{if isset($uid)}uit{else}in{/if}schakelen/{$crid}/{$voorkeur->getVanUid()}" class="knop post voorkeur-{if isset($uid)}in{else}uit{/if}geschakeld">
		<input type="checkbox"
