...
<td id="voorkeur-cell-{$voorkeur->getVanUid()}-{$crid}"
	...
	<a href="{Instellingen::get('taken', 'url')}/{if isset($uid)}uit{else}in{/if}schakelen/{$crid}/{$voorkeur->getVanUid()}" class="knop post voorkeur-{if isset($uid)}in{else}uit{/if}geschakeld">
		<input type="checkbox"
