...
*}
{strip}
<tr id="taak-datum-summery-{$datum}" class="taak-datum-summery{if strtotime($datum) < strtotime('-1 day')}{if !isset($show) and !$prullenbak}  taak-datum-oud{/if} taak-oud{/if}"{if isset($show)} style="display: none;"{/if} onclick="taken_toggle_datum('{$datum}');">
	...
	<th colspan="6" style="background-color: {cycle values="#F0F0F0,#FAFAFA"};">
