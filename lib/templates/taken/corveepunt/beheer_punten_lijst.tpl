...
<tr id="punten-row-{$puntenlijst.lid->getUid()}">
	<td>{$puntenlijst.lid->getNaamLink(Instellingen::instance()->('corvee', 'weergave_ledennamen_beheer'), Instellingen::instance()->('corvee', 'weergave_link_ledennamen'))}</td>
...
{/foreach}
	<td>
		<form method="post" action="{Instellingen::instance()->('taken', 'url')}/wijzigpunten/{$puntenlijst.lid->getUid()}" class="Formulier InlineForm">
	...
	<td>
		<form method="post" action="{Instellingen::instance()->('taken', 'url')}/wijzigbonus/{$puntenlijst.lid->getUid()}" class="Formulier InlineForm">
