...
<tr id="abonnement-row-{$uid}">
	{foreach name=loop from=$abonnementen item=abonnement}
		{if $smarty.foreach.loop.first}
			<td>{$abonnement->getLid()->getNaamLink(Instellingen::instance()->('maaltijden', 'weergave_ledennamen_beheer'), Instellingen::instance()->('maaltijden', 'weergave_link_ledennamen'))}</td>
		{/if}
