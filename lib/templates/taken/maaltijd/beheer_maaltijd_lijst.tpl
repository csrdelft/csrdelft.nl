...
{/if}
	...
	<td class="col-del">
{if $maaltijd->getIsVerwijderd()}
		<a href="{$globals.taken_module}/verwijder/{$maaltijd->getMaaltijdId()}" title="Maaltijd definitief verwijderen" class="knop post confirm">{icon get="cross"}</a>
{else}
		<a href="{$globals.taken_module}/verwijder/{$maaltijd->getMaaltijdId()}" title="Maaltijd naar prullenbak verplaatsen" class="knop post">{icon get="bin_closed"}</a>
{/if}
	</td>
</tr>
{/strip}