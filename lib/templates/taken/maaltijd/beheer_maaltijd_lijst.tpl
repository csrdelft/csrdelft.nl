...
{elseif $maaltijd->getIsVerwijderd()}
		<a href="{Instellingen::instance()->('taken', 'url')}/herstel/{$maaltijd->getMaaltijdId()}" title="Maaltijd herstellen" class="knop post">{icon get="arrow_undo"}</a>
{else}
		<a href="{Instellingen::instance()->('taken', 'url')}/bewerk/{$maaltijd->getMaaltijdId()}" title="Maaltijd wijzigen" class="knop post popup">{icon get="pencil"}</a>
...
{/if}
	...
	<td>
		<a href="{Instellingen::instance()->('taken', 'url')}/fiscaal/{$maaltijd->getMaaltijdId()}" title="Toon fiscale maaltijdlijst&#013;Maaltijdprijs: &euro; {$maaltijd->getPrijs()|string_format:"%.2f"}" class="knop">{icon get="money_euro"}</a>
...
{if !$maaltijd->getIsVerwijderd() and $maaltijd->getArchief() === null}
		<div style="float: right;">
			<a href="{Instellingen::instance()->('taken', 'url')}/anderaanmelden/{$maaltijd->getMaaltijdId()}" title="Aanmelding toevoegen" class="knop post popup">{icon get="user_add"}</a>
			<a href="{Instellingen::instance()->('taken', 'url')}/anderafmelden/{$maaltijd->getMaaltijdId()}" title="Aanmelding verwijderen" class="knop post popup">{icon get="user_delete"}</a>
		</div>
{/if}
...
{else}
	...
	<td class="maaltijd-gesloten">
		<a href="{Instellingen::instance()->('taken', 'url')}/open/{$maaltijd->getMaaltijdId()}" title="Heropen deze maaltijd" class="knop post">Gesloten</a>
	{else}
	<td class="maaltijd-open">
		<a href="{Instellingen::instance()->('taken', 'url')}/sluit/{$maaltijd->getMaaltijdId()}" title="Sluit deze maaltijd" class="knop post">Open</a>
	{/if}
{/if}
	</td>
	<td class="col-del">
{if $maaltijd->getIsVerwijderd()}
		<a href="{Instellingen::instance()->('taken', 'url')}/verwijder/{$maaltijd->getMaaltijdId()}" title="Maaltijd definitief verwijderen" class="knop post confirm range"><input type=checkbox id="box-{$maaltijd->getMaaltijdId()}" name="del-maaltijd" /> {icon get="cross"}</a>
{else}
		<a href="{Instellingen::instance()->('taken', 'url')}/verwijder/{$maaltijd->getMaaltijdId()}" title="Maaltijd naar de prullenbak verplaatsen" class="knop post range"><input type=checkbox id="box-{$maaltijd->getMaaltijdId()}" name="del-maaltijd" /> {icon get="bin_closed"}</a>
{/if}
	</td>
</tr>
{/strip}