...
class ForumContent extends TemplateView {
			...
			if (is_array($aZoekResultaten)) {
				...
				foreach ($aZoekResultaten as $aZoekResultaat) {
					...
					if (strlen($sPostFragment) >= $iFragmentLengte) {
						...
						if ($iEersteTermPos < (.5 * $iFragmentLengte)) {
							$iBegin = 0;
						} else {
							$iBegin = $iEersteTermPos - (.5 * $iFragmentLengte);
						}
						...
						if ($iBegin != 0) {
							$sPostFragment = '...' . trim($sPostFragment);
						};
					}
					...
					if ($aZoekResultaat['aantal'] != 1) {
						echo ' <em>(' . $aZoekResultaat['aantal'] . ' berichten in dit onderwerp)</em>';
					}
	...
	function view() {
		switch ($this->actie) {
			case 'recent': $this->lastPosts();
				break;
			case 'rss': $this->rssFeed();
				break;
			case 'lastposts_belangrijk': $this->lastPostsZijbalkBelangrijk();
				break;
			case 'lastposts': $this->lastPostsZijbalk();
				break;
			case 'lastposts_zelf': $this->lastPostsZijbalk(true);
				break;
			case 'zoeken': $this->zoeken();
				break;
			default: $this->viewCategories();
				break;
		}
	}
