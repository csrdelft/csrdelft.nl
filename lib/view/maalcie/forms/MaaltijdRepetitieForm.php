...
class MaaltijdRepetitieForm extends ModalForm
{
	...
	public function __construct($model, $verplaats = null)
	{
		...
		$fields[] = new RequiredTextField(
			'standaard_titel',
			$model->standaard_titel,
			'Standaard titel',
			255
		);
		$fields[] = new TimeObjectField(
			'standaard_tijd',
			$model->standaard_tijd,
			'Standaard tijd',
			15
		);
		$fields['dag'] = new WeekdagField(
			'dag_vd_week',
			$model->dag_vd_week,
			'Dag v/d week'
		);
		$fields['dag']->title =
			'Als de periode ongelijk is aan 7 is dit de start-dag bij het aanmaken van periodieke maaltijden';
		$fields[] = new IntField(
			'periode_in_dagen',
			$model->periode_in_dagen,
			'Periode (in dagen)',
			0,
			183
		);
		$fields['abo'] = new JaNeeField(
			'abonneerbaar',
			$model->abonneerbaar,
			'Abonneerbaar'
		);
		if ($model->mlt_repetitie_id !== 0) {
			$fields['abo']->onchange =
				"if (!this.checked && $(this).attr('origvalue') == 1) if (!confirm('Alle abonnementen zullen worden verwijderd!')) this.checked = true;";
		}
		...
		$fields[] = new RechtenField(
			'abonnement_filter',
			$model->abonnement_filter,
			'Aanmeldrestrictie'
		);

		$bijwerken = new FormulierKnop(
			'/maaltijden/repetities/bijwerken/' . $model->mlt_repetitie_id,
			'submit',
			'Alles bijwerken',
