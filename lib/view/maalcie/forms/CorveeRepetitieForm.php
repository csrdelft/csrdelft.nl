	...
	public function __construct(CorveeRepetitie $repetitie)
	{
		parent::__construct(
			...
			'/corvee/repetities/opslaan' .
				($repetitie->crv_repetitie_id ? '/' . $repetitie->crv_repetitie_id : '')
		);
		...
		$functieNamen = ContainerFacade::getContainer()
			->get(CorveeFunctiesRepository::class)
			->getAlleFuncties(); // grouped by functie_id
		...
		$mlt_repetities = ContainerFacade::getContainer()
			->get(MaaltijdRepetitiesRepository::class)
			->getAlleRepetities();
		$repetitieNamen = ['' => ''];
		...
		$fields = [];
		$fields['fid'] = new DoctrineEntityField(
			'corveeFunctie',
			$repetitie->corveeFunctie,
			'Functie',
			CorveeFunctie::class,
			'/corvee/functies/suggesties?q='
		);
		$fields['fid']->onchange =
			$functiePunten . "$('#field_standaard_punten').val(punten[this.value]);";
		$fields['fid']->required = true;
		$fields[] = new WeekdagField(
			'dag_vd_week',
			$repetitie->dag_vd_week,
			'Dag v/d week'
		);
		$fields['dag'] = new IntField(
			'periode_in_dagen',
		...
		$fields['dag']->title =
			'Als de periode ongelijk is aan 7 is dit de start-dag bij het aanmaken van periodiek corvee';
		$fields['vrk'] = new JaNeeField(
			'voorkeurbaar',
			$repetitie->voorkeurbaar,
			'Voorkeurbaar'
		);
		if ($repetitie->crv_repetitie_id !== 0) {
			$fields['vrk']->onchange =
				"if (!this.checked && $(this).attr('origvalue') == 1) if (!confirm('Alle voorkeuren zullen worden verwijderd!')) this.checked = true;";
		}
		...
		$fields[] = new IntField(
			'standaard_aantal',
			$repetitie->standaard_aantal,
			'Aantal corveeërs',
			1,
			10
		);

		$bijwerken = new FormulierKnop(
			'/corvee/repetities/bijwerken/' . $repetitie->crv_repetitie_id,
			'submit',
			'Alles bijwerken',
