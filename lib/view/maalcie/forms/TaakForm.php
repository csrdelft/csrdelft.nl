...
class TaakForm extends ModalForm
{
	public function __construct(CorveeTaak $taak, $action)
	{
		...
		$fields['fid']->onchange =
			$functiePunten . "$('.punten_field').val(punten[this.value]);";
		$fields['fid']->required = true;
		$fields['lid'] = new LidObjectField('profiel', $taak->profiel, 'Naam');
		$fields['lid']->title =
			'Bij het wijzigen van het toegewezen lid worden ook de corveepunten aan het nieuwe lid gegeven.';
		...
		$fields['ptn'] = new RequiredIntField(
			'punten',
			$taak->punten,
			'Punten',
			0,
		...
		$fields['crid'] = new DoctrineEntityField(
			'corveeRepetitie',
			$taak->corveeRepetitie,
			'',
			CorveeRepetitie::class,
			''
		);
		$fields['crid']->readonly = true;
		$fields['crid']->hidden = true;
		$fields['mid'] = new DoctrineEntityField(
			'maaltijd',
			$taak->maaltijd,
			'Gekoppelde maaltijd',
			Maaltijd::class,
			'/maaltijden/beheer/suggesties?q='
		);
