...
class RememberLoginForm extends ModalForm
{
	public function __construct(RememberLogin $remember)
	{
		parent::__construct(
			$remember,
			'/session/remember',
			'Automatisch inloggen vanaf huidig apparaat',
			true
		);
		...
		$fields[] = new HiddenField(
			'DataTableSelection',
			$remember->id ? $remember->id . '@rememberlogin.csrdelft.nl' : null
		);
		$fields[] = new HtmlComment(
			'<div class="dikgedrukt">Gebruik deze functie alleen voor een veilig apparaat op een veilige locatie.</div>'
		);
