...
class AgendaItemForm implements FormulierTypeInterface
{
	...
	public function createFormulier(FormulierBuilder $builder, $data, $options = []): void {
		...
		$fields = [];
		$fields['titel'] = new RequiredAutocompleteField(
			'titel',
			$data->titel,
			'Titel'
		);
		$fields['titel']->suggestions[] = [
			'Kring',
			'Lezing',
			'Werkgroep',
			'Eetplan',
		...
		$fields['begin_moment'] = new DateTimeObjectField(
			'begin_moment',
			$data->begin_moment,
			'Begin moment'
		);
		$fields['begin_moment']->required = true;
		$fields['eind_moment'] = new DateTimeObjectField(
			'eind_moment',
			$data->eind_moment,
			'Eind moment'
		);
		...
		$fields['r'] = new RequiredRechtenField(
			'rechten_bekijken',
			$data->rechten_bekijken,
			'Zichtbaar voor'
		);
		...
		$fields['b'] = new TextareaField(
			'beschrijving',
			$data->beschrijving,
			'Beschrijving'
		);
		$fields['b']->title =
			'Extra info als de cursor boven de titel gehouden wordt';

		$builder->addFields($fields);

		$formKnoppen = new FormDefaultKnoppen();
		if ($options['actie'] === 'toevoegen') {
			$doorgaan = new FormulierKnop(
				'/agenda/toevoegen/doorgaan',
				'submit',
				'Opslaan en doorgaan',
				'Opslaan & nog een agenda item toevoegen',
				'add'
			);
