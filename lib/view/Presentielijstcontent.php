 ...
 * Presentielijstcontent.php
 *
 * @deprecated
 */
class Presentielijstcontent implements View, ToResponse {
	use ToHtmlResponse;
	...
	function getHtml() {
		...
		foreach ($this->aLeden as $aLid) {
			...
			$sReturn .= '<tr><td class="naam">' . $aLid->getNaam('presentielijst') . '</td>';
	...
	function getUrl() {
		$sReturn = 'presentielijst?goederen=' . urlencode($this->getGoederen()) .
	...
	function view() {
		echo '<h1>' . $this->getTitel() . '</h1>
			<form id="presentielijst" action="presentielijst" method="get">
			<fieldset>
		...
		if (isset($_GET['toon'])) {
			echo '<a href="' . $this->getUrl() . 'iframe">Alleen de presentielijst</a><br />';
