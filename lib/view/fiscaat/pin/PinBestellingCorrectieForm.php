...
abstract class PinBestellingCorrectieForm extends ModalForm {
	...
	public function __construct($pinTransactieMatch = null) {
		...
		if (!$pinTransactieMatch) {
			...
			$commentOud = $pinTransactieMatch->bestelling->comment ?: $this->voltooidDeelwoord . ' op ' . date_format_intl(date_create_immutable(), DATE_FORMAT);
			$internOud = $pinTransactieMatch->notitie ?: '';
			$commentNieuw = $this->commentNieuw . ' pinbetaling ' . PinTransactieMatch::renderMoment($pinTransactieMatch->bestelling->moment, false);
		}
