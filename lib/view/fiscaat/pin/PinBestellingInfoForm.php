...
class PinBestellingInfoForm extends ModalForm
{
	...
	public function __construct($pinTransactieMatch)
	{
		...
		if ($pinTransactieMatch->transactie !== null) {
			$fields['pinMoment'] = new DateTimeObjectField(
				'pinMoment',
				$pinTransactieMatch->transactie->datetime,
				'Transactie moment'
			);
			$fields['pinMoment']->readonly = true;
		}
		if ($pinTransactieMatch->bestelling !== null) {
			$civiSaldo = ContainerFacade::getContainer()
				->get(CiviSaldoRepository::class)
				->find($pinTransactieMatch->bestelling->uid);
			$fields['lid'] = new DoctrineEntityField(
				'uid',
				$civiSaldo,
				'Account',
				CiviSaldo::class,
			...
			$fields['moment'] = new DateTimeObjectField(
				'moment',
				$pinTransactieMatch->bestelling->moment,
				'Bestelling moment'
			);
			$fields['moment']->readonly = true;
			$fields['comment'] = new TextField(
				'comment',
				$pinTransactieMatch->bestelling->comment,
				'Externe notitie'
			);
		}
		$fields['intern'] = new TextareaField(
			'intern',
			$pinTransactieMatch->notitie,
			'Interne notitie'
		);
		if ($pinTransactieMatch->bestelling !== null) {
			$fields[] = new CiviBestellingInhoudTable(
