...
class ActiviteitForm implements FormulierTypeInterface {
	...
	public function createFormulier(FormulierBuilder $builder, $data, $options = []) {
		...
		$fields['capaciteit'] = new IntField('capaciteit', $data->getRawCapaciteit(), 'Capaciteit', 0);
		$fields['capaciteit']->placeholder = $data->getReeks()->getRawCapaciteit();
		$fields['rechtenAanmelden'] = new RechtenField('rechtenAanmelden', $data->getRawRechtenAanmelden(), 'Rechten aanmelden');
		$fields['rechtenAanmelden']->placeholder = $data->getReeks()->getRawRechtenAanmelden();
		$fields['rechtenLijstBekijken'] = new RechtenField('rechtenLijstBekijken', $data->getRawRechtenLijstBekijken(), 'Rechten lijst bekijken');
		$fields['rechtenLijstBekijken']->placeholder = $data->getReeks()->getRawRechtenLijstBekijken();
		$fields['rechtenLijstBeheren'] = new RechtenField('rechtenLijstBeheren', $data->getRawRechtenLijstBeheren(), 'Rechten lijst beheren');
