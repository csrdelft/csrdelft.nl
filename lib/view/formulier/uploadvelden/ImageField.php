	...
	public function validate() {
		...
		if ($this->getModel() instanceof Afbeelding AND in_array($this->getModel()->mimetype, $this->filterMime)) {
			...
			if ($resize) {
				...
				if ($this->vierkant) {
					$command = getenv('IMAGEMAGICK') . ' ' . escapeshellarg($directory . $filename) . ' -thumbnail 150x150^ -gravity center -extent 150x150 -format jpg -quality 80 ' . escapeshellarg($resized);
				} else {
					$command = getenv('IMAGEMAGICK') . ' ' . escapeshellarg($directory . $filename) . ' -resize ' . $percent . '% -format jpg -quality 85 ' . escapeshellarg($resized);
				}
