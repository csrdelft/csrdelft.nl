...
class BbLocatie extends BbTag {
	...
	public function render() {
		...
		if (trim(htmlspecialchars($address)) == '') {
			...
			$maps = '<iframe height="' . $this->height . '" frameborder="0" style="border:0;width:100%" src="https://www.google.com/maps/embed/v1/place?q=' . urlencode(htmlspecialchars($address)) . '&key=' . getenv('GOOGLE_EMBED_KEY') . '"></iframe>';
		}
		$map = $maps;
