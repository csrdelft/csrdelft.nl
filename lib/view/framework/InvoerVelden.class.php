...
abstract class InputField implements FormElement, Validator {
	...
	public function getLabel() {
		if (!empty($this->description)) {
			...
			$help = '';
			if ($this->title) {
				$help = '<div class="help"><img width="16" height="16" class="icon hoverIntentContent" alt="?" src="/plaetjes/famfamfam/help.png"></div>';
			}
...
class WachtwoordWijzigenField extends InputField {
	...
	public function validate() {
		...
		if (!$this->require_current OR ! empty($new)) {
            ...
            } elseif (!preg_match('/^.{8,}$/', $new)) {
                $this->error = 'Het nieuwe wachtwoord moet minimaal 8 tekens bevatten.';
