...
class GroepLedenImportController extends AbstractController
{
	...
	public function upload(Request $request, Session $session): Response
	{
		...
		if (
			array_keys($data[0]) !== ['groepID', 'uid', 'opmerking']
		) {
	...
	public function controle(
		...
		if (empty($data)) {
			return $this->quickMelding('Er ging iets fout bij het inladen van de CSV');
		}

		// Ga regels langs
		$groeplidRegels = GroepLedenImportDTO::convert($profielRepository, $groepRepository, $data);
		$aantalSucces = count(array_filter($groeplidRegels, function(GroepLedenImportDTO $dto) {
			return $dto->succes;
		}));
	...
	public function verwerk(
		...
		try {
			foreach ($data as $regel) {
				$regel = new GroepLedenImportDTO($profielRepository, $groepRepository, $regel);
