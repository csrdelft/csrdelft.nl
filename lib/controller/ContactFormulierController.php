...
";
	...
	public function civitasproducties(): PlainView
	{
		...
		$bericht = $this->renderView(
			'mail/bericht/civitasproductiesbestelling.mail.twig',
			[
				'naam' => $naam,
				'voornaam' => $voornaam,
				'email' => $email,
		...
		$mail = new Mail(
			[
				$_ENV['EMAIL_CIVITASPRODUCTIES'] => 'Civitas producties',
				$email => $voornaam . ' ' . $naam,
			],
			'Bevestiging film ticket bestelling.',
			$bericht
		);
		...
		return new PlainView(
			'Bestelling verzonden. Er is een bevestiging gestuurd naar uw emailadres, binnenkort zult ontvangt u meer informatie.'
		);
	}
	...
	private function bevatUrl($opmerking): bool
	{
