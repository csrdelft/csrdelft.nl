...
class SponsorAffiliateDownloadCommand extends Command
{
	...
	protected function execute(
		...
		if (
			!$this->sponsorSlHost ||
			!$this->sponsorClubId ||
			!$this->sponsorUserAgent
		) {
			$output->writeln(
				'Zorg ervoor dat SPONSOR_SL_HOST, SPONSOR_CLUBID en SPONSOR_USERAGENT gezet zijn in .env'
			);
		...
		//Steps
		$PAGE_URL =
			$this->sponsorSlHost . '/api/?call=webshops_club_extension&club=';
