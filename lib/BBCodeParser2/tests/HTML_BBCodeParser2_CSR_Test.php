...
class HTML_BBCodeParser2_CSR_Test extends PHPUnit_Framework_TestCase
{
	var $corefilters;
	var $allfilters;

	public function __construct() {
		$core = 'Basic,Email,Extended,Images,Links,Lists';
		$csr = 'Bbcodeoff';
		$this->corefilters = array('filters' => $core);
		$this->allfilters = array('filters' => $core . ',' . $csr);

	}


	function testCSRfilters(){


	}

	function testBBcodeoff() {
		$bbc = new HTML_BBCodeParser2($this->allfilters);

		$this->assertEquals('[b]aa&lt;i&gt;bbb&lt;/i&gt;cc[/b]<strong>tekst</strong>', $bbc->qparse('[tekst][b]aa<i>bbb</i>cc[/b][/tekst][b]tekst[/b]'));
		$this->assertEquals('[b]aa&lt;i&gt;bbb&lt;/i&gt;cc[/b]', $bbc->qparse('[ubboff][b]aa<i>bbb</i>cc[/b][/ubboff]'));

	}
	function testGeenbbcodeoff() {
		$bbc = new HTML_BBCodeParser2($this->corefilters);

		$this->assertEquals('[tekst]<strong>aa&lt;i&gt;bbb&lt;/i&gt;cc</strong>[/tekst]<strong>tekst</strong>', $bbc->qparse('[tekst][b]aa<i>bbb</i>cc[/b][/tekst][b]tekst[/b]'));
		$this->assertEquals('[ubboff]<strong>aa&lt;i&gt;bbb&lt;/i&gt;cc</strong>[/ubboff]', $bbc->qparse('[ubboff][b]aa<i>bbb</i>cc[/b][/ubboff]'));
	}
