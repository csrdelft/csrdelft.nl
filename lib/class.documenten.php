...
//	}
	...
	function updateDocument($id, $title, $cat) {
		...
		if(isset($id, $title, $cat) && is_numeric($id) && is_numeric($cat) && is_string($title)){
			$homedir='/srv/www/www.csrdelft.nl/data/leden/documenten/uploads/';
			...
			$ready=true;
			if($oldCat != $newCat){
				$ready=rename($homedir.$oldCat.'/'.$filename,$homedir.$newCat.'/'.$filename);
			}
	...
	function deleteDocument($docid) {
		$homedir='/srv/www/www.csrdelft.nl/data/leden/documenten/uploads/';
		$trashdir='/srv/www/www.csrdelft.nl/data/leden/documenten/trash/';
		...
		if(!$rFiles) {
			...
			while($aFilename=$this->_db->next($rFiles)) {
				...
				$move=rename($homedir.$postpath,$trashdir.$postpath);
				if(!$move) { // moving failed: do rollback and return false
					foreach($aFilesDone as $filename) {
						$postpath=$catid.'/'.$filename;
						rename($trashdir.$postpath,$homedir.$postpath);
					}
					return false;
	...
	function performUpload($docid){
		if(!isset($docid, $_FILES['file'])){ return false; }
