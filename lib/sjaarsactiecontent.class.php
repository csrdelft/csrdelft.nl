...
class SjaarsactieContent extends TemplateView {
	### private ###
	...
	function viewNewActieForm() {
		$naam = $beschrijving = '';
		$limiet = 10;
		if (isset($_POST['actieNaam'])) {
			$naam = trim(htmlspecialchars($_POST['actieNaam']));
		}
		if (isset($_POST['beschrijving'])) {
			$beschrijving = trim(htmlspecialchars($_POST['beschrijving']));
		}
			...
			foreach ($aSjaarsacties as $aSjaarsactie) {
				...
				if (is_array($aSjaarsjes) AND count($aSjaarsjes) != 0) {
					...
					foreach ($aSjaarsjes as $aSjaars) {
						...
						if ($aSjaars['uid'] == $this->_sjaarsactie->_lid->getUid()) {
							$bAlAangemeld = true;
						}
					}
