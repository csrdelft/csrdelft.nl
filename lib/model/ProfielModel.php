...
class ProfielModel extends CachedPersistenceModel {
	...
	protected function retrieveByPrimaryKey(array $primary_key_values) {
		if (startsWith($primary_key_values[0], '19') && !LoginModel::mag('commissie:NovCie,commissie:PubCie'))  return false;
		return parent::retrieveByPrimaryKey($primary_key_values); // TODO: Change the autogenerated stub
	}
	...
	public static function existsUid($uid) {
		if (!LoginModel::mag('commissie:NovCie,commissie:PubCie') && startsWith($uid, '19')) {
			return false;
		}
	...
	public static function existsNick($nick) {
		return Database::instance()->sqlExists(static::instance()->getTableName(), 'nickname = ? AND uid NOT LIKE \'19%\'', array($nick));
	}

	public static function existsDuck($duck) {
		return Database::instance()->sqlExists(static::instance()->getTableName(), 'duckname = ? AND uid NOT LIKE \'19%\'', array($duck));
	}
	...
	public function find($criteria = null, array $criteria_params = [], $group_by = null, $order_by = null, $limit = null, $start = 0) {
		if (!LoginModel::mag('commissie:NovCie,commissie:PubCie')) $criteria = "($criteria) AND uid NOT LIKE '19%'";
		return parent::find($criteria, $criteria_params, $group_by, $order_by, $limit, $start);
	}
}
