...
class LoginModel extends PersistenceModel implements Validator {
	...
	private function loginWeb($uid, $pass, $tokenOK = false, $lockIP = false) {
		...
		if ($uid != 'x999') {
			...
			if (!$field->validate()) {
				if (!startsWith(REQUEST_URI, '/wachtwoord') AND REQUEST_URI !== '/endsu' AND ! startsWith(REQUEST_URI, '/tools/css.php') AND ! startsWith(REQUEST_URI, '/tools/js.php')) {
					setMelding('Uw wachtwoord ' . $field->getName() . '_new' . '[' . filter_input(INPUT_POST, $field->getName() . '_new', FILTER_SANITIZE_STRING) . ']is onveilig: ' . str_replace('nieuwe', 'huidige', $field->getError()), 2);
