...
class VerjaardagenModel {
	...
	public static function get($maand) {
		return ProfielModel::instance()->find("status in ('S_LID', 'S_GASTLID', 'S_NOVIET', 'S_KRINGEL') AND EXTRACT(MONTH FROM gebdatum) = ? AND uid NOT LIKE '17%'", array($maand), null, 'EXTRACT(DAY FROM gebdatum)');
	}
	...
	public static function getKomende($aantal = 10) {
		return ProfielModel::instance()->find("status IN ('S_LID', 'S_GASTLID', 'S_NOVIET', 'S_KRINGEL') AND NOT gebdatum = '0000-00-00' AND uid NOT LIKE '17%'", array(), null, "DATE_ADD(
					gebdatum,
	...
	public static function getTussen($van, $tot, $limiet = null) {
		...
		return ProfielModel::instance()->find("status IN ('S_LID', 'S_GASTLID', 'S_NOVIET', 'S_KRINGEL') AND NOT gebdatum = '0000-00-00' AND (
            ...
            OR
            (CONCAT(?, SUBSTRING(gebdatum, 5)) >= ? AND CONCAT(?, SUBSTRING(gebdatum, 5)) < ?)
        ) AND uid NOT LIKE '17%'", array($vanjaar, $van, $vanjaar, $tot, $totjaar, $van, $totjaar, $tot), null, "DATE_ADD(
					gebdatum,
					INTERVAL TIMESTAMPDIFF(
						year,
						ADDDATE(gebdatum, INTERVAL 1 DAY),
