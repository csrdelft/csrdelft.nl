...
# Aangepast voor gebruik in het CMS van OogOpslag Internet
# Juni 2005 Hans van Kranenburg
...
{
	$tpl_filename = 'bbcode/bbcode.tpl';
	#$tpl = fread(fopen($tpl_filename, 'r'), filesize($tpl_filename));
	# vanaf PHP 4.3.0:
	$tpl = file_get_contents($tpl_filename, true);
...
function prepare_bbcode_template($bbcode_tpl)
{
	...
	# LET OP derde regel staat bewust op de derde regel telkens!
	$bbcode_tpl['imgrel'] = str_replace('{URL}', '\\1', $bbcode_tpl['img']);
 	$bbcode_tpl['img'] = str_replace('{URL}', '\\1', $bbcode_tpl['img']);

	$bbcode_tpl['imglrel'] = str_replace('{URL}', '\\1', $bbcode_tpl['imgl']);
	$bbcode_tpl['imgl'] = str_replace('{URL}', '\\1', $bbcode_tpl['imgl']);

	$bbcode_tpl['imgrrel'] = str_replace('{URL}', '\\1', $bbcode_tpl['imgr']);
	$bbcode_tpl['imgr'] = str_replace('{URL}', '\\1', $bbcode_tpl['imgr']);

	$bbcode_tpl['imgcrel'] = str_replace('{URL}', '\\1', $bbcode_tpl['imgc']);
	$bbcode_tpl['imgc'] = str_replace('{URL}', '\\1', $bbcode_tpl['imgc']);
	...
	$bbcode_tpl['url2rel'] = str_replace('{URL}', '\\1', $bbcode_tpl['urlrel']);
	$bbcode_tpl['url2rel'] = str_replace('{DESCRIPTION}', '\\1', $bbcode_tpl['url2rel']);
	...
	$bbcode_tpl['url4rel'] = str_replace('{URL}', '\\1', $bbcode_tpl['urlrel']);
	$bbcode_tpl['url4rel'] = str_replace('{DESCRIPTION}', '\\2', $bbcode_tpl['url4rel']);
...
function bbencode_second_pass($text, $uid)
{
	...
	// Maak de inhoud van de tabellen aan, geen rekening gehouden met geneste tabellen
	// haal eerst tekst in de tabel op
	$matches = array();
	if (preg_match("/\[tabel:$uid\](.*)\[\/tabel:$uid\]/si", $text, $matches) != 0) {
		$tabeltext = klus_tabel($matches[1]);
		$text = preg_replace("/(\[tabel:$uid\])(.*)(\[\/tabel:$uid\])/si", "\\1{$tabeltext}\\3", $text);
	}

	// [tabel] and [/tabel]
	$text = str_replace("[tabel:$uid]", $bbcode_tpl['tabel_open'], $text);
	$text = str_replace("[/tabel:$uid]", $bbcode_tpl['tabel_close'], $text);
	...
	$text = str_replace("[kopje1:$uid]", $bbcode_tpl['kopje1_open'], $text);
	...
	$patterns[] = "#\[imgl:$uid\]([^?].*?)\[/imgl:$uid\]#i";
	$replacements[] = $bbcode_tpl['imgl'];
	$patterns[] = "#\[imgr:$uid\]([^?].*?)\[/imgr:$uid\]#i";
	$replacements[] = $bbcode_tpl['imgr'];
	$patterns[] = "#\[imgc:$uid\]([^?].*?)\[/imgc:$uid\]#i";
	$replacements[] = $bbcode_tpl['imgc'];
	...
	// [url]folder/page.htm[/url] code
	$patterns[] = "#\[url\]([^ \"\n\r\t<]*)\[/url\]#is";
	$replacements[] = $bbcode_tpl['url2rel'];
	...
	// [url=folder/file.jpg]phpBB[/url] code.. (no xxxx:// prefix).
	$patterns[] = "#\[url=([^ \"\n\r\t<]*)\](.*?)\[/url\]#is";
	$replacements[] = $bbcode_tpl['url4rel'];
...
function bbencode_first_pass($text, $uid)
{
	...
	// [tabel] en [/tabel]
	$text = bbencode_first_pass_pda($text, $uid, "[tabel]", "[/tabel]", '',  false, '');
...
/**
 * Hiermee gaan we de inhoud van de tabel in elkaar klussen. We bekijken de regels die
 * tussen [tabel] en [/tabel] staan een voor een. In de tabel zetten we regels die geen
 * [:] bevatten (1) in een cel met colspan 2, en regels die wel [:] bevatten (2), splitsen
 * we daarop en zetten we de delen in twee cellen.
 *
 ...
 *               1  ---------- 1 -> doe niets
 *             /               2 -> </td></tr><tr><td>
 * (nu in mode)     (nieuwe mode)      (actie)
 *             \               1 -> </td></tr><tr><td colspan="2">
 *               2  ---------- 2 -> </td></tr><tr><td>
 *
 * daarna: 1 -> zet tekst neer
 *         2 -> vervang [:] door </td><td>
 *
 */
function klus_tabel($text) {
	$text = str_replace("<br />", "", $text);
	$text = str_replace("<br>", "", $text);
	# de stukjes tabel
	$tabel[0][1] = "<tr><td class=\"bbcodetabel\" colspan=\"3\">\n";
	$tabel[0][2] = "<tr><td class=\"bbcodetabel\">\n";
	$tabel[1][1] = "";
	$tabel[1][2] = "</td></tr><tr><td class=\"bbcodetabel\">\n";
	$tabel[2][1] = "</td></tr><tr><td class=\"bbcodetabel\" colspan=\"3\">\n";
	$tabel[2][2] = "</td></tr><tr><td class=\"bbcodetabel\">\n";
	$tabel[1][0] = "</td></tr>\n";
	$tabel[2][0] = "</td></tr>\n";

	# eerst de tekst splitten op de regeleinden
	$regels = preg_split("/[\r\n]/", $text);
	# als de eerste regel leeg is (dat is meestal de enter na [tabel])
	# dan die regel weggooien
	if (trim($regels[0]) == "") array_shift($regels);
	# als de laatste regel ook leeg is (dat is dan de enter voor [/tabel])
	# dan ook weggooien
	if (trim($regels[count($regels)-1]) == "") array_pop($regels);
	#var_dump($regels);
	if (count($regels) == 0) return "";
	# nu gaan we de regels een voor een bekijken en hoppen heen en weer tussen
	# een colspan=2 of twee cellen
	$mode = 0;
	$output = '';
	foreach ($regels as $regel) {
		$regel = trim($regel);
		$to_mode = (strpos($regel,'[:]') === false) ? 1 : 2;
		$output .= $tabel[$mode][$to_mode];
		switch ($to_mode) {
			case 1: $output .= $regel . "<br />\n"; break;
			case 2:
				$regel = str_replace("<br />", "", $regel);
				$output .= preg_replace("/\[:\]/", "</td><td width=\"20\"></td><td>", $regel,1);
				break;
		}
		$mode = $to_mode;
	}
