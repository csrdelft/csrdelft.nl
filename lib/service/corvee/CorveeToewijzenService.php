...
class CorveeToewijzenService
{
	...
	public function getSuggesties(CorveeTaak $taak): array
	{
		...
		if ($functie->kwalificatie_benodigd) {
			// laad alleen gekwalificeerde leden
			...
			foreach ($functie->kwalificaties as $kwali) {
				...
				if (!$profiel) {
					throw new CsrGebruikerException(
						sprintf('Lid met uid "%s" bestaat niet.', $profiel->uid)
					);
				}
				...
				if (array_key_exists($uid, $vrijstellingen)) {
					...
					if (
						$taak->datum >= $vrijstelling->begin_datum &&
						$taak->datum <= $vrijstelling->eind_datum
					) {
				...
				$corveePuntenOverzichten[
					$uid
				] = $this->corveePuntenService->loadPuntenVoorLid($profiel, [
					$functie->functie_id => $functie,
				]);
				$corveePuntenOverzichten[$uid]->aantal =
			...
			foreach ($corveePuntenOverzichten as $uid => $punten) {
				if (array_key_exists($uid, $vrijstellingen)) {
					...
					$datum = $taak->datum;
					if (
						$datum >= $vrijstelling->begin_datum &&
						$datum <= $vrijstelling->eind_datum
					) {
					...
					if ($vrijstelling !== null && $datum < $vrijstelling->begin_datum) {
						$corveePuntenOverzichten[
							$uid
						]->prognose -= $vrijstelling->getPunten();
					}
				}
			}
