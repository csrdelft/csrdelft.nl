...
class CorveeTakenRepository extends AbstractRepository
{
	...
	public function getKomendeTakenVoorLid(Profiel $profiel)
	{
		return $this->createQueryBuilder('ct')
			->where(
				'ct.verwijderd = false and ct.profiel = :profiel and ct.datum >= :datum'
			)
		...
		return $this->getEntityManager()->transactional(function () use ($taak) {
			if ($taak->taak_id === null) {
				...
				$oldTaak = $this->getEntityManager()
					->getUnitOfWork()
					->getOriginalEntityData($taak);

				if (
					$oldTaak['corveeFunctie']->functie_id !=
	 ...
	 * @throws Throwable
	 */
	public function maakRepetitieTaken(
		CorveeRepetitie $repetitie,
		$beginDatum,
		$eindDatum,
		$maaltijd = null
	...
	public function newRepetitieTaken(
		...
		while ($datum <= $eindDatum) {
			...
			$datum = $datum->add(
				new DateInterval("P{$repetitie->periode_in_dagen}D")
			);
		}
	...
	{
		return $this->getEntityManager()->transactional(function () use ($repetitie, $verplaats): RepetitieTakenUpdateDTO {
			$taken = $this->findBy([
				'verwijderd' => false,
			...
			$takenPerDatum = []; // taken per datum indien geen maaltijd
			$takenPerMaaltijd = []; // taken per maaltijd
			$maaltijden = ContainerFacade::getContainer()
				->get(MaaltijdenRepository::class)
				->getKomendeRepetitieMaaltijden($repetitie->mlt_repetitie_id);
			/** @var Maaltijd[] $maaltijdenById */
			$maaltijdenById = [];
			foreach ($maaltijden as $maaltijd) {
				$takenPerMaaltijd[$maaltijd->maaltijd_id] = [];
			...
			foreach ($taken as $taak) {
				...
				if ($taak->maaltijd) {
					if (array_key_exists($taak->maaltijd->maaltijd_id, $maaltijdenById)) {
						// do not change if not komende repetitie maaltijd
