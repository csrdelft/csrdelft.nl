...
const withTags: GalleryDecorator = (constructor) =>
	class extends constructor {
		private get imageElement() {
			try {
				return select<HTMLImageElement>(
		...
		private moveTagDivs() {
			this.tagContainer
				...
				.forEach((t: HTMLElement) =>
					this.moveTag(t, JSON.parse(t.dataset.tag ?? '{}'))
				);
		}
		...
		private moveTagForm() {
			...
			const pos = this.getScreenPos(
				JSON.parse(this.tagFormDiv.dataset.tagPosition ?? '{}') as Position
			);
		...
		private drawTag(tag: Tag) {
			...
			const tagDiv = createElement(
				`<div id="${tag.keyword}" class="fototag" title="${tag.name}"></div>`,
				{
					style: {
			...
			const btnDelTag = createElement(
				`<a id="btnDelTag" tabindex="-1" class="dropdown-item"><i class="fas fa-user-xmark" aria-hidden="true"></i>&nbsp;Etiket verwijderen</a>`
			);
		...
		private duringTagMode() {
			if (this.preview.size !== Size.contain) {
				// if zoomed in
				alert(
					'Je kunt niet inzoomen tijdens het etiketteren, dat werkt nog niet.'
				);
			}
			if (this.tagFormDiv) {
				this.exitTagForm();
			...
			const left = this.previewElement.querySelector(
				'.j-gallery-left'
			) as HTMLElement;
			const right = this.previewElement.querySelector(
				'.j-gallery-right'
			) as HTMLElement;
		...
		private newTagStart(e: MouseEvent) {
			...
			const windowMouseMoveListener = (e2: MouseEvent) => {
				...
				if (this.tagFormDiv) {
					const sizeInput = this.tagFormDiv.querySelector(
						'input[name="size"]'
					) as HTMLInputElement;
