...
export const initDropzone = (el: HTMLFormElement): void => {
	const { naam, accept, deleteUrl, maxsize, coverUrl, existingUrl } =
		el.dataset;
	const thisDropzone = new Dropzone(el, {
		...
		dictFallbackMessage:
			'Je browser ondersteund niet het slepen van bestanden.',
		dictFallbackText:
			'Please use the fallback form below to upload your files like in the olden days.',
		dictFileTooBig:
			'Te groot bestand: ({{filesize}}MiB). Maximum: {{maxFilesize}}MiB.',
		...
		init: function () {
			this.on('addedfile', function (file) {
				const coverBtn = Dropzone.createElement(
					'<a class="btn" title="Stel deze foto in als omslag voor het album">Omslag</a>'
				);
				...
				coverBtn.addEventListener('click', async function (e) {
					...
					try {
						...
						coverBtn.replaceWith(html`<span><i class="fas fa-check" aria-hidden="true"></i>Omslag</span>`)
					} catch (e) {
						throw new Error(e);
					}
				});
	...
	const showExisting = html`<a href="#"
		><i class="fas fa-images" aria-hidden="true"></i> Toon bestaande foto's in dit album</a
	>`;
