...
export function isDataTableResponse(
	response: unknown
): response is DatatableResponse {
...
export async function initDataTable(el: HTMLElement): Promise<void> {
	...
	settingsJson.columns.forEach(
		(col: DataTables.ColumnSettings) =>
			(col.render = render[col.render as string])
	);
...
export function fnUpdateDataTable(
	tableId: string,
...
export function getApiFromSettings(
...
export const fnGetLastUpdate = ($table: JQuery) => (): number =>
	Number($table.data('lastupdate'));
export const fnSetLastUpdate =
	($table: JQuery) =>
	(lastUpdate: number): JQuery =>
		$table.data('lastupdate', lastUpdate);
/**
 ...
 * @param {jQuery} $table
 */
export const fnAjaxUpdateCallback =
	($table: JQuery) =>
	(json: DatatableResponse): unknown => {
