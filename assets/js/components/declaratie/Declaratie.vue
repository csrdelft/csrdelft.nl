<template>
	<div class="declaratie">
		...
		<div v-if="declaratie.statusData.magBeoordelen && declaratie.status !== 'concept'" class="field">
		...
		<div class="field">
			...
			<select id="categorie" v-model="declaratie.categorie" :disabled="veldenDisabled">
				<option disabled />
				<option v-for="(categorieID, categorie) in categorieen" :key="'categorie-' + categorieID" :value="categorieID">
					{{ categorie }}
				</option>
			</select>
		</div>
		...
		<div v-if="declaratie.betaalwijze === 'voorgeschoten' && !declaratie.eigenRekening" class="field">
			<label for="rekening">IBAN</label>
			<input id="rekening" v-model="declaratie.rekening" type="text" :disabled="veldenDisabled" maxlength="255" />
		</div>
		...
		<div
			...
			class="field"
		>
			<label v-if="declaratie.betaalwijze === 'voorgeschoten'" for="tnv">Ten name van</label>
			<label v-else for="tnv">Bij bedrijf</label>
			<input id="tnv" v-model="declaratie.tnv" type="text" :disabled="veldenDisabled" maxlength="255" />
		</div>
		...
		<div v-if="bonUploaden || !heeftBonnen" class="bonnen bon-upload">
			<div class="inhoud">
				...
				<p>
					Upload je bon of factuur als PDF of goed leesbare foto. Neem daarna de bedragen van de bon of het factuur
					over.
				</p>
				<div class="buttons">
					...
					<template v-else>
						...
						<input id="fileUpload" type="file" accept=".jpg,.jpeg,.png,.pdf" @change="uploadBon($event.target.files)" />
						<button v-if="heeftBonnen" class="open" @click="bonUploaden = false">Annuleren</button>
					</template>
				</div>
			</div>
		</div>
		...
		<div v-if="!bonUploaden && heeftBonnen" class="bonnen bonnen-weergave">
			<div class="lijst">
				<div v-for="(bon, bonIndex) in declaratie.bonnen" :key="'bon-' + bonIndex" class="bon">
					<div v-if="bonIndex !== geselecteerdeBon" class="bon-collapsed" @click="geselecteerdeBon = bonIndex">
						<div class="left">
					...
					<div v-else class="bon-selected">
						...
						<div class="bon-regels">
							...
							<div v-for="(regel, index) in bon.regels" :key="'regel-' + bonIndex + '-' + index" class="regels-row">
								<div class="field">
									<input v-model="regel.omschrijving" type="text" :disabled="veldenDisabled" maxlength="255" />
								</div>
								<div class="field">
									<money v-model="regel.bedrag" v-money="money" style="text-align: right" :disabled="veldenDisabled" />
								</div>
								<div class="field">
							...
							<div v-if="!veldenDisabled" class="regels-row nieuw" @click="nieuweRegel(bon)">
								<div class="field">
							...
							<div v-if="berekening(bon).btw[9] !== 0" class="regels-row totaal">
								<div class="onderdeel">Btw 9%</div>
								<div class="bedrag">
									{{ berekening(bon).btw[9] | bedrag }}
								</div>
							</div>
							<div v-if="berekening(bon).btw[21] !== 0" class="regels-row totaal">
				...
				<div v-if="!veldenDisabled" class="nieuwe-bon" @click="bonUploaden = true">
					<i class="fa fa-plus-circle" />
				</div>
			</div>
			<div class="voorbeeld">
		...
		<div class="field">
			...
			<textarea id="opmerkingen" v-model="declaratie.opmerkingen" :disabled="veldenDisabled" />
		</div>
		...
		<div v-for="(error, index) in errors" :key="'error-' + index" class="field alert alert-warning" role="alert">
			{{ error }}
		</div>
		...
		<div v-if="!veldenDisabled && !submitting && !editing" class="save-buttons">
			...
			<button class="confirm" :disabled="submitting" @click="declaratieOpslaan(true)">Declaratie indienen</button>
		</div>

		<div v-if="editing && !submitting" class="save-buttons">
			<button class="afkeuren" :disabled="submitting" @click="reload()">Wijzigingen annuleren</button>
			<button class="confirm" :disabled="submitting" @click="declaratieOpslaan(true)">Opslaan</button>
		</div>
...
export default class DeclaratieVue extends Vue {
	...
	private get veldenDisabled() {
		return this.submitting || (this.declaratie.status !== 'concept' && !this.editing);
	}
