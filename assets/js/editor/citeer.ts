...
const getForumPost = (sel: Selection): HTMLElement | null => {
	if (sel.type == 'Range' && sel.rangeCount) {
		...
		const htmlContainer =
			selContainer instanceof HTMLElement
				? selContainer
				: selContainer.parentElement;
...
const getCiteerDiv = () => {
	...
	citeerDiv.querySelector('.btn').addEventListener('click', () => {
		...
		editor.dispatch(
			editor.state.tr.insert(editor.state.selection.head, citaatNode)
		);
