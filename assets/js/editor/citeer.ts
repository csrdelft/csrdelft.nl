...
const getCiteerDiv = () => {
	...
	const citeerDiv = html` <div id="${CITEER_DIV_ID}" class="card position-absolute" style="display: none">
...
const citeerSelectionHandler = () => {
	...
	if (window.currentEditor && selectionInForumPost && !window.getSelection().isCollapsed) {
		...
		const popper = createPopper({ getBoundingClientRect: getSelectionBoundingRect }, citeerDiv, {
			placement: 'bottom',
		});
		popper.update();
	} else {
