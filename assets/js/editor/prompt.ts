...
export function openPrompt<T = any>(options: PromptOptions<T>): void {
	const submitButton = html` <button
		type="submit"
		class="${prefix}-submit btn btn-primary"
	>
		OK
	</button>`;
	const cancelButton = html` <button
		type="button"
		class="${prefix}-cancel btn btn-primary"
	>
		Cancel
	</button>`;
	...
	Object.entries(options.fields).forEach(([name, field]) => {
		formBody.appendChild(html` <div class="mb-3 row">
			<label class="col-sm-2 col-form-label" for="${name}"
				>${field.options.label}${field.options.required
	...
	formFooter.appendChild(html` <div class="${prefix}-buttons">
