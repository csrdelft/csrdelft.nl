		...
		fetch(`/groepen/${type}/${node.attrs[attr]}/info`)
			...
			.then((json) => {
				...
				for (const lid of json.leden) {
					ledenDiv.appendChild(html`<a href="#" title="${lid.naam}" class="lidLink"
						><img class="pasfoto" src="/profiel/pasfoto/${lid.uid}.jpg" alt="Pasfoto van ${lid.naam}"
					/></a>`);
				}
...
<div class="clear">&nbsp;</div>
	...
	parseDOM: [
		{
			tag: `div[data-${type}`,
			getAttrs: (dom: HTMLElement) => ({ [attr]: dom.dataset[type], naam: dom.dataset.naam }),
		},
	],
});
